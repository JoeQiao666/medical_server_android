...
@ActivityFragmentInject(contentViewId = R.layout.activity_in,
        toolbarTitle = R.string.activity_in,
        isShowLeftBtn = false
)
public class InActivity extends BaseActivity<InOutContract.InPresenter> implements CompoundButton.OnCheckedChangeListener, InOutContract.InView {
...
//        params.put("departmentId", UserData.getInstance().getLoginData().user.getDepartmentId());
    ...
    @Override
    protected InOutContract.InPresenter initPresenter() {
        return new InPresenter(this);
    }
